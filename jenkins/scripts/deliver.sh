...
set -x
NAME=$(mvn -q -DforceStdout help:evaluate -Dexpression=project.name | sed 's/\x1b\[[0-9;]*m//g' | tr -d '\r')
...
set -x
VERSION=$(mvn -q -DforceStdout help:evaluate -Dexpression=project.version | sed 's/\x1b\[[0-9;]*m//g' | tr -d '\r')
